...
export function SiteNav({ locale = "en" }: { locale?: Locale }) {
  ...
  return (
    <header className="relative z-50 flex justify-center px-4 pt-[22px] sm:pt-[34px]">
      <nav
        aria-label="Main"
        className="flex w-full items-center justify-between rounded-nav border border-border bg-card px-[22px] py-[11px] sm:w-auto sm:justify-start sm:gap-x-6"
      >
        <Link
      ...
      {open && (
        <div
          id="mobile-menu"
          className="absolute left-4 right-4 top-full mt-[6px] rounded-card border border-border bg-card px-[22px] py-[6px] sm:hidden"
        >
          {t.links.map((link, i) => (
            <Link
              key={link.path}
